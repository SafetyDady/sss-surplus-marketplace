...
// ไม่ใช้ ProtectedRoute เพื่อข้าม logic การตรวจสอบสิทธิ์
export default function AdminDashboardDirect() {
  ...
  React.useEffect(() => {
    console.log('Admin Dashboard Direct - adminData:', adminData);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {adminData && (
        <div className="bg-yellow-100 p-4 mb-4">
          ...
          <p className="text-yellow-800">Role: {adminData.role || 'ไม่พบข้อมูล'}</p>
          <p className="text-yellow-800">Email: {adminData.email || 'ไม่พบข้อมูล'}</p>
          <p className="text-yellow-800">Permissions: {adminData.permissions?.join(', ') || 'ไม่พบข้อมูล'}</p>
        </div>
      )}
