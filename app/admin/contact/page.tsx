...
export default function AdminContactPage() {
  ...
  if (loading) {
    return (
      <ProtectedRoute requiredPermission="contact_management">
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="p-6">
              ...
              {activeTab === 'vendors' && (
                <div>
                  <div className="text-center py-8">
                    <p className="text-gray-500">ข้อมูล Vendor Applications จะแสดงที่นี่</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {selectedMessage && (
            <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
              <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div className="mt-3">
                  <h3 className="text-lg font-medium text-gray-900 mb-4">รายละเอียดข้อความ</h3>
                  
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700">ผู้ส่ง:</label>
                      <p className="text-sm text-gray-900">{selectedMessage.senderInfo?.name}</p>
                      <p className="text-sm text-gray-500">{selectedMessage.senderInfo?.email}</p>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700">หัวข้อ:</label>
                      <p className="text-sm text-gray-900">{selectedMessage.messageInfo?.subject}</p>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700">ข้อความ:</label>
                      <p className="text-sm text-gray-900">{selectedMessage.messageInfo?.message}</p>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">ตอบกลับ:</label>
                      <textarea
                        value={replyText}
