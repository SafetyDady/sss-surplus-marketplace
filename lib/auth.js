  ...
  return onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      try {
        ...
        if (customClaims.role && ['admin', 'super_admin', 'moderator'].includes(customClaims.role)) {
          callback({ user, adminData: { role: customClaims.role, permissions: customClaims.permissions || [] } });
        } else {
          // Not an admin
          callback(null);
        }
